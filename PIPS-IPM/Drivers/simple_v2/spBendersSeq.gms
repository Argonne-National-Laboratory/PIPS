...
* Get current values for the type_mult(type) parameter for each scenario subproblem
        ...
        obj_sub(kk,scen) = zsub.l;

        bcoef_plant(kk,scen,rp)  = sum(t, eq_plant_capacity.m(t,rp) * avail(t,rp) * %RESOLUTION%);
        bcoef_stor(kk,scen,rs)   = sum(t, eq_storage_capacity.m(t,rs) );
        bcoef_link(kk,scen,net)  = sum(t, eq_link_capacity.m(t,net) * %RESOLUTION% );
...
display "-----------------------------------------------------------------",
        ...
        "-----------------------------------------------------------------";
display rep;
Execute_Unload 'spBendersSeq_results_%NBREGIONS%.gdx',done,t_e,t_e_k,trr,clscen,kcl_cut,k_incmap,obj_total,obj_master,obj_model,obj_sub,bconst,bconst_check,bcoef_plant,bcoef_stor,bcoef_link,p_PLANT_ADD_CAP,p_STORAGE_ADD_CAP,p_LINK_ADD_CAP,final_PLANT_ADD_CAP,final_STORAGE_ADD_CAP,final_LINK_ADD_CAP;
