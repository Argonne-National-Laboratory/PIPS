...
#define SCALE_OBJ 1000.0 //1000.0
#define SCALE_C   500.0 //500.0 //5000.0
...
{ 
  ...
  if( argc>1 && iargv == argc && goodUsage ) {
    ...
    if(solveWith!=0 && 
       solveWith!=1 && solveWith!=2 && solveWith!=4 &&
       solveWith!=5 && solveWith!=6 &&
       solveWith!=10 && solveWith!=11 && solveWith!=12 && solveWith!=13 && 
       solveWith!=20 ) {
      cerr << "Solver(s) is not recognized/specified.\n";
      return 1;
    }
  } else {
    cerr << "Usage: " << argv[0] << "  --solve num [--datadir path] [--printx]\n";
    cerr << "\t--solve num    - solver used: \n";
    cerr << "\t\t\t -  0 OOQP's default\n";
    cerr << "\t\t\t -  1 STOCH augmented\n";
    cerr << "\t\t\t - 10 STOCH reduced-augmented \n";
...
void stochSolve(int argc, char *argv[], PbData& pbData, int printx, int solveWith)
{
  ...
  if(size<2 && solveWith-solveWith/10*10!=0) {
    cerr << "Could not deploy preconditioner since it needs at least 2 processes.\n";
    return;
  }
...
{ 
  ...
  int N=data->N; int T=data->T; int S=data->S;
...
{ 
  PbData* data = (PbData*)user_data;
  int N=data->N; int S=data->S; int NW=data->NW;
  ...
  if(id==0) {
    *nnz = 0;
    ...
    //13-14-15
    //////////////////////////////////////////////
    *nnz += 2*N*(S-1) + 2*N*(S-1) + 2*NW*(S-1); //!4
...
int fQ(void* user_data, int id, int* krowM, int* jcolM, double* M)
{
  ...
  if(id==0) {
    int n = data->get_nx(1);
    int beg_krow=0;
    int end_krow=3*NT; assert(end_krow<n);

    for(int i=beg_krow; i<end_krow; i++) krowM[i]=0;

    beg_krow+=3*NT; end_krow+=S*N; assert(end_krow<n);
    
    //only cp gets quadratic terms
    for(int i=beg_krow; i<end_krow; i++) {
      int colIdx = i-beg_krow; 
      krowM[i] = colIdx; 
      jcolM[colIdx] = i;
      
      M[colIdx] = fabs(b[colIdx % N])/100/SCALE_OBJ;
      //printf("%g ", M[colIdx]);
    }

    int jcolIdx=end_krow-beg_krow; assert(jcolIdx==S*N); 
    beg_krow+=S*N; end_krow+=S*N+S*NW; assert(end_krow==n);
    
    for(int i=beg_krow; i<=end_krow; i++) krowM[i]=jcolIdx;
    
  } else {
    int n = data->get_nx(2);

    int NTm1=N*(T-1); double nrmNumbers[NTm1];
    normalRand(0, fabs(b[0])/100/50, NTm1, &nrmNumbers[0]);

    for(int i=0; i<N*(T-1); i++) {
      krowM[i] = i; //only i since beg_row is 0; 
      jcolM[i] = i; //only i since beg_row is 0; 
      M[i] = (fabs(b[i%N])/100 + nrmNumbers[i])/SCALE_OBJ;
    }
    int jcolIdx=N*(T-1);
    for(int i=N*(T-1); i<n; i++) krowM[i]=jcolIdx;
 ...
 /*int fQ(void* user_data, int id, int* krowM, int* jcolM, double* M)
{
  ...
  if(id==0) {
    ...
    double nrmNumbers[n];
    normalRand(0, fabs(b[0])/100/50, n, &nrmNumbers[0]);

    for(int i=0; i<n; i++) {
      krowM[i] = i; 
      jcolM[i] = i; 
      M[i] = fabs(b[i%N])/100 + nrmNumbers[i];
    }
    krowM[n]=n;
...
int fc(void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    ...
    for(int s=0; s<S; s++)
    for(int j=0; j<N; j++) {
      vec[3*NT + s*N+j]    = b[j]/S;   // pc[j,1,s] //!4
      //!2 vec[i+N]    = 1.0;   // cp[j,1]
      vec[3*NT+S*N + s*N+j]  = 0.0;   // pcmax
    }
 
    assert(3*NT+2*S*N+S*NW==len); 
    for(int i=3*NT+2*S*N; i<3*NT+2*S*N+S*NW; i++)
      vec[i]      = 0.0;   // pwind[j,1]//!4
  } else {
...
int fA(void* user_data, int id, int* krowM, int* jcolM, double* M)
{
  ...
  if(id==0) {
    ...
    */
    ...
    //13 14 15                                //!4
    ///////////////////////////////////////////////
    // pc[j,1,s] - pc[j,1,1] = 0 same for pcmax and pcwind
    for(int s=1; s<S; s++) {
      for(int j=0; j<N; j++) {
	krowM[ind_krowM] = ind_jcolM;
	
	jcolM[ind_jcolM] = 3*N*T + j;
	M[ind_jcolM] = -1.0;
	ind_jcolM++;

	jcolM[ind_jcolM] = 3*N*T + s*N+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;

	krowM[ind_krowM+1] = ind_jcolM; ind_krowM++;
      }
    }
    //pcmax
    for(int s=1; s<S; s++) {
	...
	jcolM[ind_jcolM] = 3*N*T+S*N + s*N+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;
	...
	jcolM[ind_jcolM] = 3*N*T+2*S*N + j;
	M[ind_jcolM] = -1.0;
	ind_jcolM++;
	
	jcolM[ind_jcolM] = 3*N*T+2*S*N + s*NW+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;
...
{ 
  ...
  if(id==0) {
    ...
    for(int s=0; s<S; s++) {
      krowM[rowind] = colind;
      for(int j=0; j<N; j++) {
	jcolM[colind] = 3*N*T + s*N+j;
	M[colind] = 1.0;
	colind++;
      }
      for(int j=0; j<NW; j++) {
	jcolM[colind] = 3*N*T+2*N*S+ s*NW+j;
	M[colind] = -data->NWT;
	colind++;
      }      
      krowM[rowind+1] = colind;
    ...
    */
    ...
    for(int s=0; s<S; s++) {
      krowM[rowind] = colind;
      for(int i=0; i<N; i++) {
	jcolM[colind] = 3*NT+N*S + s*N+i;
	M[colind] = 1.0;
	colind++;
      }

      for(int i=0; i<NW; i++) {
	jcolM[colind] = 3*NT+2*N*S + s*NW+i;
	M[colind] = -data->NWT;
	colind++;
      }
      krowM[rowind+1] = colind;
    ...
    */
	...
	jcolM[colind] = j; 
	M[colind] = -data->Pmin[j]; //v[j,1]
	colind++;

	jcolM[colind] = 3*N*T + s*N+j; //pc[j,1,s]
	M[colind] = 1.0;
	colind++;
    ...
    */
	...
	jcolM[colind] = 3*N*T + s*N+j; //pc
	M[colind] = -1.0;
	colind++;

	jcolM[colind] = 3*N*T+S*N + s*N+j; //pcmax[j,1,s]
	M[colind] = 1.0;
	colind++; 
	...
	jcolM[colind] = j;
	M[colind] = data->Pmax[j]; //v[j,1]
	colind++;

	jcolM[colind] = 3*N*T+N*S + s*N+j; //pcmax[j,1,s]
	M[colind] = -1.0;
	colind++;
    ...
    */
    ...
    for(int j=0; j<N; j++) {
      ...
      jcolM[colind] = 3*N*T + ss*N+j; M[colind] = 1.0;           colind++; //pc[j,1,s] //!4
      krowM[rowind+1] = colind; rowind++;
    }
    ...
    for(int j=0; j<N; j++) {
      ...
      jcolM[colind] = 3*N*T+S*N + ss*N+j; M[colind] = -1.0;          colind++; //pcmax[j,1,s]
      krowM[rowind+1] = colind; rowind++;
    }
    // --- k>2 -> do it T-2 times. We start at k=1 to be compatible with the indices.
    for(int k=1; k<T-1; k++) {
...
{
  PbData* data = (PbData*)user_data;
  int N=data->N; int NW=data->NW; int S=data->S;
  int* G = data->G; int* L = data->L;

  if(id==0) {
    assert(len==(2*N+NW)*(S-1));
...
{  
  ...
  if(id==0) {
    ...
    // ------ 5 ------
    double* demand=data->demand; int NWT=data->NWT; int S=data->S;
    double* wind_scen = new double[NW]; // wind scenario for k=0, s
    for(int s=0; s<S; s++) {
      data->loadWindScenario(s, 0, wind_scen, NW);
    ...
    */
    ...
    // ------ 8 ------
    for(int sj=0; sj<S*N; sj++) vec[it++] = 0.0;  
    //!4 for(int j=0; j<N; j++) vec[it++] = 0.0;
    ...
    //!4 for(int j=0; j<N; j++) vec[it++] = 0.0;
#ifdef DEBUG
    assert(it==N*T*ND+N*T +S+S+ S*(N+N+N));
#endif
...
int fixlow(void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    ...
    //!4 for(int i=3*NT; i<3*NT+N; i++) vec[i] = 1.0; //lower limit on pc[j,1]
    for(int i=3*NT; i<3*NT+S*N; i++) vec[i] = 1.0; //lower limit on pc[j,1,s]
    //!4 for(int i=3*NT+N; i<3*NT+2*N; i++) vec[i] = 1.0; //lower limit on pcmax[j,1]
    for(int i=3*NT+S*N; i<3*NT+2*S*N; i++) vec[i] = 1.0; //lower limit on pcmax[j,1,s]
    //!4 for(int i=3*NT+2*N; i<3*NT+2*N+NW; i++) vec[i] = 1.0; //lower limit on pwind[j,1]
    for(int i=3*NT+2*S*N; i<3*NT+2*S*N+S*NW; i++) vec[i] = 1.0; //lower limit on pwind[j,1,s]
  } else {
...
int fxupp (void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    ...
    int nx = data->get_nx(1);
    assert(nx==len);
    assert(nx==3*NT+2*S*N+S*NW);
#endif
    for(int i=0; i<N*T; i++) vec[i]=1.0; //upper on v
    for(int i=N*T; i<3*NT+2*S*N; i++) vec[i]=0.0; // no limit
    
    for(int s=0;s<S;s++)
      data->loadWindScenario(s, 0, &vec[3*NT+2*S*N+s*NW], NW); 
    //for(int i=3*NT+2*N; i<3*NT+2*N+NW; i++) vec[i]=0.0;
...
int fixupp(void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    for(int i=0; i<N*T; i++) vec[i]=1.0; //upper on v
    for(int i=N*T; i<3*NT+2*S*N; i++) vec[i]=0.0; // no limit //!2 //!4
    for(int i=3*NT+2*S*N; i<3*NT+2*S*N+S*NW; i++) vec[i]=1.0; //pwind upper limit //!4
  } else {
    for(int i=0; i<N*(T-1); i++) vec[i]=0.0; // no upper limit for pc//! 2
...
{ return stage==1?3*N*T+S*(2*N+NW) : (T-1)*(2*N+NW); }
			      ...
			      double* data, int noWindUnits)
{
  ...
  if(!scen_loaded) {
    ...
    fstream file_op(strFile.c_str(), ios::in); assert(file_op.is_open());
