...
void PresolveData::computeRowMinActivity(SystemType system_type, int node, BlockType block_type, int row)
{
   ...
   for( int j = Amat.rowptr[row].start; j < Amat.rowptr[row].end; j++)
   {
      ...
      if( PIPSisLT(0.0, entry) )
      {
         if( ixlow_root[col] != 0.0)
            actmin_part += entry * xlow_root[col];
      }
      // add entry * upper_bound to min_partact
      else
      {
         if( ixupp_root[col] != 0.0 )
            actmin_part += entry * xupp_root[col];
      }
   }
...
void PresolveData::computeRowMaxActivity(SystemType system_type, int node, BlockType block_type, int row)
{
   ...
   for( int j = Amat.rowptr[row].start; j < Amat.rowptr[row].end; j++)
   {
      ...
      {
         if( ixupp_root[col] != 0.0)
            actmax_part += entry * xupp_root[col];
      }
      // add entry * upper_bound to min_partact
      else
      {
         if( ixlow_root[col] != 0.0 )
            actmax_part += entry * xlow_root[col];
      }
   }
...
#ifdef TRACK_COLUMN
   if(NODE == node && COLUMN == col)
      std::cout << "col " << COLUMN << " is linking and gets it's activities updated" << std::endl;
#endif
...
#ifdef TRACK_COLUMN
   if(NODE == node && COLUMN == col)
      std::cout << "col " << COLUMN << " is non-linking and gets it's activities updated" << std::endl;
#endif
      /* Bmat, Blmat */
      /* Bmat */
...
void PresolveData::updateRowActivitiesLinkingConsBlock(SystemType system_type, int node, BlockType block_type, int col, double bound, double old_bound, bool upper)
{
  ...
  SimpleVector& actmin_part_chgs = (system_type == EQUALITY_SYSTEM) ? *actmin_eq_chgs : *actmin_ineq_chgs;
   ...
   if(NODE == node && COLUMN == col)
   {
      std::cout << "actually updating with a new bound of " << bound << " and an old one of " << old_bound << " (upper was " << upper << ")" << std::endl;
