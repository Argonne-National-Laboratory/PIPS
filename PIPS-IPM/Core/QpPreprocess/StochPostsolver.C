...
bool StochPostsolver::postsolveBoundsTightened(sVars& original_vars, int reduction_idx) const
{
   const int type = reductions.at(reduction_idx);
   ...
   else
   {
      ...
      else if(is_upper_bound)
      {
         ...
         assert(PIPSisZero(getSimpleVecFromColStochVec(original_vars.w, col)));
         slack = new_bound - curr_x;
      }
      else
      {
         assert(PIPSisLE(0, curr_x - new_bound));
         assert(PIPSisZero(getSimpleVecFromColStochVec(original_vars.v, col)));
      ...
      if(is_upper_bound)
      {
         ...
         if(!PIPSisZeroFeas(phi * slack))
         {
            ...
            original_vars.phi->axpy(1.0, *tmp_pos);
            original_vars.gamma->axpy(1.0, *tmp_neg);
...
//         const double coeff = float_values.at(first_float_val + 1); TODO: not needed - remove
   ...
   getSimpleVecFromColStochVec(original_vars.v, col) = (xlow_old == INF_NEG) ? 0 : value - xlow_old;
   getSimpleVecFromColStochVec(original_vars.w, col) = (xupp_old == INF_POS) ? 0 : xupp_old - value;
...
bool StochPostsolver::postsolveSingletonInequalityRow(sVars& original_vars, int reduction_idx) const
{
   ...
   const double old_bound = (xupp_new != INF_POS) ? xupp_old : xlow_old;
...
//         assert( PIPSisRelLEFeas( val_col1, xupp_col1 ) );
   ...
   assert( xlow_implied != INF_NEG );
   assert( xupp_implied != INF_POS );
