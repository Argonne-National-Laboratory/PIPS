...
QpGenStochLinsysLeaf::QpGenStochLinsysLeaf(QpGenStoch *factory_, QpGenStochData* prob,
					   OoqpVector* dd_, 
					   OoqpVector* dq_,
...
QpGenStochLinsysLeaf::~QpGenStochLinsysLeaf()
{

}

void QpGenStochLinsysLeaf::factor2(QpGenStochData *prob, Variables *vars)
{
  ...
  */
  ...
  stochNode->resMon.recFactTmChildren_start();
  solver->matrixChanged();
  stochNode->resMon.recFactTmChildren_stop();
}

void QpGenStochLinsysLeaf::putXDiagonal( OoqpVector& xdiag_ )
{
  StochVector& xdiag = dynamic_cast<StochVector&>(xdiag_);
  kkt->atPutDiagonal( 0, *xdiag.vec );
}

void QpGenStochLinsysLeaf::putZDiagonal( OoqpVector& zdiag_)
{
  StochVector& zdiag = dynamic_cast<StochVector&>(zdiag_);
  kkt->atPutDiagonal( locnx+locmy, *zdiag.vec );
}

void QpGenStochLinsysLeaf::Lsolve  (  QpGenStochData *prob, OoqpVector& x_in )
{
...
void QpGenStochLinsysLeaf::Dsolve  (  QpGenStochData *prob, OoqpVector& x_in )
{
  StochVector& x = dynamic_cast<StochVector&>(x_in);
  assert(x.children.size()==0);
...
void QpGenStochLinsysLeaf::Ltsolve (  QpGenStochData *prob, OoqpVector& x_in )
{
  StochVector& x = dynamic_cast<StochVector&>(x_in);
  assert(x.children.size()==0);
...
void QpGenStochLinsysLeaf::Ltsolve2( QpGenStochData *prob, StochVector& x, SimpleVector& xp)
{
...
void QpGenStochLinsysLeaf::sync()
{ assert(false); }


void QpGenStochLinsysLeaf::deleteChildren()
{ }

void mySymAtPutSubmatrix(SymMatrix& kkt_, 
