...
#ifndef QPGENSCADRIVER
#define QPGENSCADRIVER
...
int qpgensca_solve( int argc, char * argv[],
		 SOLVER * /* dum1 */, FORMULATION * /* dum2 */)
{
  int bcomm;
  MPI_Comm mpicomm = MPI_COMM_WORLD;
  //int nprow = 2, npcol = 2, nb = 4;
  int nb = 50, dims[2] = {0,0}, nprocs;
  COMMINFO cinfo;
  ...
  blacs_pinfo_(&bcomm,&bcomm); // need to call this to link for some reason
  bcomm = mpicomm;
  
  blacs_gridinit_(&bcomm, "Row-major", &dims[0], &dims[1]);
  
  
  initcomm(cinfo, mpicomm, bcomm, nb, dims[0], dims[1]);  
  
  if (cinfo.mype == 0) {
    cout << dims[0] << " by " << dims[1] << " processor grid" << endl;
  }
...
#endif
    //cout << "read in, solving...\n";
    int result = s->solve(prob,vars, resid);
    if( 0 == result && cinfo.mype == 0 ) {
