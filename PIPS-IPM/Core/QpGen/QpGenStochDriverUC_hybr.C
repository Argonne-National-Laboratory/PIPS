...
public:
  ...
  // number of scenarios for which the non-anticipativity constraints are 
  // explicitly enforced
  int r; 
...
{ 
  ...
  int N=data->N; int ND=data->ND; int T=data->T; int S=data->S; int NW=data->NW; int r=data->r;
...
{ 
  PbData* data = (PbData*)user_data;
  int N=data->N; int ND=data->ND; int T=data->T; int NW=data->NW; int S=data->S; int r=data->r;
...
{
  PbData* data = (PbData*)user_data;
  int N=data->N; int T=data->T; int NW=data->NW; int S=data->S; int r = data->r;
  ...
  if(id==0) {
    ...
    for(int s=0; s<r+1; s++)
    for(int j=0; j<N; j++) {
      vec[3*NT + s*N+j]    = b[j]/(r+1);   // pc[j,1,s] //!4
      //!2 vec[i+N]    = 1.0;   // cp[j,1]
      vec[3*NT+(r+1)*N + s*N+j]  = 0.0;   // pcmax
    }
 
    assert(3*NT+2*(r+1)*N+(r+1)*NW==len); 
    for(int i=3*NT+2*(r+1)*N; i<3*NT+2*(r+1)*N+(r+1)*NW; i++)
...
int fA(void* user_data, int id, int* krowM, int* jcolM, double* M)
{
  ...
  if(id==0) {
    ...
    */
	...
	jcolM[ind_jcolM] = 3*N*T + (s-S+r+1)*N+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;
	...
	jcolM[ind_jcolM] = 3*N*T+(r+1)*N + j;
	M[ind_jcolM] = -1.0;
	ind_jcolM++;
	
	jcolM[ind_jcolM] = 3*N*T+(r+1)*N + (s-S+r+1)*N+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;
	...
	jcolM[ind_jcolM] = 3*N*T+2*(r+1)*N + j;
	M[ind_jcolM] = -1.0;
	ind_jcolM++;
	
	jcolM[ind_jcolM] = 3*N*T+2*(r+1)*N + (s-S+r+1)*NW+j;
	M[ind_jcolM] = 1.0;
	ind_jcolM++;
...
{ 
  PbData* data = (PbData*)user_data;
  int N=data->N; int T=data->T; int NW=data->NW; int ND=data->ND; int S = data->S; int r=data->r;
  double** K = data->K;
  int NT=N*T;
  ...
  if(id==0) {
    ...
    for(int s=S-r; s<=S; s++) {
      krowM[rowind] = colind;
      for(int j=0; j<N; j++) {
	jcolM[colind] = 3*N*T + (s-S+r)*N+j;
	M[colind] = 1.0;
	colind++;
      }
      for(int j=0; j<NW; j++) {
	jcolM[colind] = 3*N*T+2*N*(r+1) + (s-S+r)*NW+j;
	M[colind] = -data->NWT;
	colind++;
      }      
      krowM[rowind+1] = colind;
    ...
    */
    ...
    for(int s=S-r; s<=S; s++) {
      krowM[rowind] = colind;
      for(int i=0; i<N; i++) {
	jcolM[colind] = 3*NT+N*(r+1) + (s-S+r)*N+i;
	M[colind] = 1.0;
	colind++;
      }

      for(int i=0; i<NW; i++) {
	jcolM[colind] = 3*NT+2*N*(r+1) + (s-S+r)*NW+i;
	M[colind] = -data->NWT;
	colind++;
      }
      krowM[rowind+1] = colind;
    ...
    */
#ifdef DEBUG
    nnz = (r+1)*(N+NW + N+NW);
    assert(rowind-row_save==2*(r+1));//4
    row_save = rowind;
    assert(colind-col_save==nnz);
    col_save = colind;
#endif
	...
	jcolM[colind] = 3*N*T + (s-S+r)*N + j; //pc[j,1,s]
	M[colind] = 1.0;
	colind++;
    ...
    */
	...
	jcolM[colind] = 3*N*T + (s-S+r)*N+j; //pc
	M[colind] = -1.0;
	colind++;

	jcolM[colind] = 3*N*T+(r+1)*N + (s-S+r)*N+j; //pcmax[j,1,s]
	M[colind] = 1.0;
	colind++; 
	...
	jcolM[colind] = 3*N*T+N*(r+1) + (s-S+r)*N+j; //pcmax[j,1,s]
	M[colind] = -1.0;
	colind++;
    ...
    */
#ifdef DEBUG
    nnz = (6*N)*(r+1);
    assert(rowind-row_save==3*N*(r+1));
    row_save = rowind;
    assert(colind-col_save==nnz);
    col_save = colind;
    fnnzC(data, id, &nnz);
    ...
    for(int j=0; j<N; j++) {
      ...
      jcolM[colind] = 3*N*T+(r+1)*N + ss*N+j; M[colind] = -1.0;          colind++; //pcmax[j,1,s]
      krowM[rowind+1] = colind; rowind++;
    }
    // --- k>2 -> do it T-2 times. We start at k=1 to be compatible with the indices.
    for(int k=1; k<T-1; k++) {
...
{  
  ...
  if(id==0) {
    ...
    // ------ 5 ------
    double* demand=data->demand; int NWT=data->NWT; int S=data->S; int r=data->r;
    double* wind_scen = new double[NW]; // wind scenario for k=0, s
    for(int s=S-r-1; s<S; s++) {
    ...
    */
    ...
    // ------ 8 ------
    for(int sj=0; sj<(r+1)*N; sj++) vec[it++] = 0.0;  
    ...
    //!4 for(int j=0; j<N; j++) vec[it++] = 0.0;
#ifdef DEBUG
    assert(it==N*T*ND+N*T + r+1 + r+1 + (r+1)*(N+N+N));
#endif
...
int fixlow(void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    ...
    for(int i=3*NT; i<3*NT+(r+1)*N; i++) vec[i] = 1.0; //lower limit on pc[j,1,s]
    //!4 for(int i=3*NT+N; i<3*NT+2*N; i++) vec[i] = 1.0; //lower limit on pcmax[j,1]
    for(int i=3*NT+(r+1)*N; i<3*NT+2*(r+1)*N; i++) vec[i] = 1.0; //lower limit on pcmax[j,1,s]
    //!4 for(int i=3*NT+2*N; i<3*NT+2*N+NW; i++) vec[i] = 1.0; //lower limit on pwind[j,1]
    for(int i=3*NT+2*(r+1)*N; i<3*NT+2*(r+1)*N+(r+1)*NW; i++) vec[i] = 1.0; //lower limit on pwind[j,1,s]
  } else {
...
int fxupp (void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    ...
    int nx = data->get_nx(1);
    assert(nx==len);
    assert(nx==3*NT+2*(r+1)*N+(r+1)*NW);
#endif
    for(int i=0; i<N*T; i++) vec[i]=1.0; //upper on v
    for(int i=N*T; i<3*NT+2*(r+1)*N; i++) vec[i]=0.0; // no limit
    
    for(int s=0;s<r+1;s++)
      data->loadWindScenario(s, 0, &vec[3*NT+2*(r+1)*N+s*NW], NW); 
...
int fixupp(void* user_data, int id, double* vec, int len)
{
  ...
  if(id==0) {
    for(int i=0; i<N*T; i++) vec[i]=1.0; //upper on v
    for(int i=N*T; i<3*NT+2*(r+1)*N; i++) vec[i]=0.0; // no limit //!2 //!4
    for(int i=3*NT+2*(r+1)*N; i<3*NT+2*(r+1)*N+(r+1)*NW; i++) vec[i]=1.0; //pwind upper limit //!4
  } else {
    for(int i=0; i<N*(T-1); i++) vec[i]=0.0; // no upper limit for pc//! 2
...
{ return stage==1?3*N*T+(r+1)*(2*N+NW) : (T-1)*(2*N+NW); }
