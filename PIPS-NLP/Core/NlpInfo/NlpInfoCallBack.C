...
{
	if(_RowMap) delete [] _RowMap;
	if (_invpMap) delete [] _invpMap;
	if(_xStatus) delete [] _xStatus;
	if(_consStatus) delete [] _consStatus;

	if (_RowMap_CLow) delete [] _RowMap_CLow;
	if (_RowMap_CUp) delete [] _RowMap_CUp;
	if (_RowMap_XLow) delete [] _RowMap_XLow;
	if (_RowMap_XUp) delete [] _RowMap_XUp;

	if (_dA_goff) free (_dA_goff);
	if (_dC_goff) free (_dC_goff);

	if (_JacAGoffTransMap) free (_JacAGoffTransMap);
	if (_JacCGoffTransMap) free (_JacCGoffTransMap);
...
{			
...

	{
		...
		for( j = 0; j < nx; j++ ) {
			//		  printf( "CP: j=%d, kColsTemp=%d, kColsTemp+1=%d \n", j, kColsTemp[j],kColsTemp[j+1]);
