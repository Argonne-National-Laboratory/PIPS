...
sNlpInfoFromNL::sNlpInfoFromNL(int nx_in,int my_in,int mz_in,int nzH_in,int nzA_in,int nzC_in)
...
sNlpInfoFromNL::sNlpInfoFromNL(sData *data_in, amplGenStochInput_AddSlack& in, const int ChildIdx)
	:sInfo(data_in)
{
  data_in->inputNlp = this;
  datarootname = data_in->datarootname;
...
void sNlpInfoFromNL::createChildren(sData *data_in, amplGenStochInput_AddSlack& in)
{
	int mype_; 
	MPI_Comm_rank(mpiComm/* MPI_COMM_WORLD*/, &mype_);
...
void sNlpInfoFromNL::ConstraintBody( NlpGenVars * vars_, OoqpVector *conEq, OoqpVector *conIneq)
{
...
void 
sNlpInfoFromNL::ConstraintBody_General( NlpGenVars * vars_, OoqpVector *conEq, OoqpVector *conIneq)
{
#ifdef TIMING
		  double tTot=MPI_Wtime();
#endif
  ...
  for(size_t it=0; it<children.size(); it++)
   children[it]->ConstraintBody(vars->children[it], local_conEq->children[it], local_conIneq->children[it]);
...
void 
sNlpInfoFromNL::ConstraintBody_DummyCon( NlpGenVars * vars_, OoqpVector *conEq, OoqpVector *conIneq)
{
#ifdef TIMING
		  double tTot=MPI_Wtime();
#endif
...
//    double *tempConInEq 	= (double*) malloc(locMz*sizeof(double));
	...
	SimpleVector& parent_X = dynamic_cast<SimpleVector&>(*(vars_X.parent->vec));
	...
	for(itVar=LocGloVarMap->begin(); itVar!=LocGloVarMap->end(); itVar++){
	  local_conEq[findEq] =  simplelocal_X[itVar->first] - parent_X[itVar->second];
	  findEq++;
	}
	assert( findEq  == locMy);
...
//    free(tempConEq);  
  ...
  for(size_t it=0; it<children.size(); it++)
   children[it]->ConstraintBody(vars->children[it], conEq_st->children[it], conIneq_st->children[it]);
...
void sNlpInfoFromNL::ObjGrad_FromSon( NlpGenVars * vars_, OoqpVector *grad_, double* tempfromPar  )
{
  ASL_pfgh *asl  = asl_local;
...
void sNlpInfoFromNL::ObjGrad_General( NlpGenVars * vars_, OoqpVector *grad_ )
{
  ...
  for(size_t it=0; it<children.size(); it++){
    children[it]->ObjGrad_FromSon(vars->children[it],sGrad->children[it], tempFromSonGrad);
  }
...
void sNlpInfoFromNL::JacFull( NlpGenVars * vars_, GenMatrix* JacA, GenMatrix* JacC) 
{
  if(gAddSlackParallelSetting==0)
...
void sNlpInfoFromNL::JacFull_General( NlpGenVars * vars_, GenMatrix* JacA, GenMatrix* JacC) 
{
#ifdef TIMING
		  double tTot=MPI_Wtime();
#endif
  ...
  if(mC+mA>0) {
	...
	double *tempAmat = (double*) malloc(Amat->numberOfNonZeros()*sizeof(double));
    double *tempBmat = (double*) malloc(Bmat->numberOfNonZeros()*sizeof(double));
    double *tempCmat = (double*) malloc(Cmat->numberOfNonZeros()*sizeof(double));
	double *tempDmat = (double*) malloc(Dmat->numberOfNonZeros()*sizeof(double));
...
// Note that we assume 1st B&D are empty
void sNlpInfoFromNL::JacFull_DummyCon( NlpGenVars * vars_, GenMatrix* JacA, GenMatrix* JacC) 
{
#ifdef TIMING
		  double tTot=MPI_Wtime();
#endif
...
//	double *tempDmat = (double*) malloc(0*sizeof(double));
	...
	getMat_ValOnly(nnzBeqLoc-parent->locNx,Bmat->M(),tempFullJac_Ampl,LocBeqLocJacGoffMap); 
	getMat_ValOnly(nnzDineqLoc,Dmat->M(),tempFullJac_Ampl,LocDineqLocJacGoffMap); 
...
void sNlpInfoFromNL::Hessian_FromSon( NlpGenVars * vars_, double* tempfromPar )
{
  ...
  if(mA+mC>0) {
	...
	double *tempX_Ampl = (double*) malloc(n_var*sizeof(double));
    double *tempH_Ampl = (double*) malloc((nnzQDiag+nnzQCross+nnzQParent)*sizeof(double));
    ...
    local_Y->negate();    local_Y->copyIntoArray(tempY);    local_Y->negate();
    local_Z->negate();    local_Z->copyIntoArray(tempZ);    local_Z->negate();
	...
	assert(Qdiag->numberOfNonZeros()==nnzQDiag && Qborder->numberOfNonZeros()==nnzQCross);
...
void sNlpInfoFromNL::Hessian_General( NlpGenVars * vars_, SymMatrix *Hess )
{
  ...
  MPI_Allreduce(tempFromSonH, tempQDiag, nnzQDiag, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
//	Qdiag->copyMtxFromDouble(Qdiag->numberOfNonZeros(),tempH);
	...
	assert(Qdiag->numberOfNonZeros()==nnzQDiag && Qborder->numberOfNonZeros()==0);
