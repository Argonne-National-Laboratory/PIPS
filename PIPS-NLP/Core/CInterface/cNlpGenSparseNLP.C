		     ...
		     int    *  ierr )
{
  ...
  *c     = (double *) malloc( nx * sizeof( double ) );
  ...
  if( *ierr != 0 ) {
    freeNlpGenSparse( c,     irowQ,  jcolQ,  dQ,
			  ...
			  int * ierr )
{
  ...
  ctx->solver  = 0;
  try {
  	NlpGenSparse * factory;
    
	factory = new NlpGenSparseWithSolver( nx, my, mz, nnzQ, nnzA, nnzC );
    ...
    FilterIPMSolver * solver = new FilterIPMSolver( factory, prob );
    //solver->monitorSelf();
		...
		int print_level, int * status_code )
{
  ...
  NlpGenHbNLPSetup( c, 	nx, 	krowQ,	jcolQ,	dQ,
			   xlow,   ixlow,  xupp,   ixupp,
  ...
  if( *status_code == 0 ) {
    ...
    NlpGenFinish( &ctx, x, gamma, phi, y, z, lambda, pi, objectiveValue,
		 status_code );
  }
	      ...
	      int print_level, int * status_code )
{
  *status_code = 0;
  ...
  if( *status_code == 0 ) {
    ...
    if( *status_code == 0 ) {
      Nlpsolvehb( c,      nx,     krowQ,   jcolQ,  dQ,
		 xlow,   ixlow,  xupp,    ixupp,
		 krowA,  my,     jcolA,   dA,     bA,
		 krowC,  mz,     jcolC,   dC,
		 clow,   iclow,  cupp,    icupp,
