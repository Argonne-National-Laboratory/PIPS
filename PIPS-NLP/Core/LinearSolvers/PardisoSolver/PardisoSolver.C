...
PardisoSolver::PardisoSolver( SparseSymMatrix * sgm, const int numOfNegEigVal_in )
{
  SetUpPardisoSolver(sgm );
...
*/
  //**************************	  added by NYC	****************************************************//	  
	//FIXME_NY: I think this is correct, set mat B and D and diag part
	// if we would like to modify all the matrix, e.g. update jacobian, it's better to set firstSolve=false in schur_solve?
	for(int r=0; r<n; r++) {
	  int colIdxTemp = 0;
	  // Msys - find the element index
	  int idxGoffMsys=-1;
	  for(int idx=krowMsys[r]; idx<krowMsys[r+1]; idx++){
		colIdxTemp =  jcolMsys[idx];
		idxGoffMsys = idx; 
	  
		// aug	- find the element index 
		int idxGoffAug=-1;
		for(int idx2=krowM[colIdxTemp]; idx2<krowM[colIdxTemp+1]; idx2++)
		  if(jcolM[idx2]==r) 
			{idxGoffAug=idx2; break;}
	  
		elementMap_BD.insert( pair<int,int>(idxGoffMsys,idxGoffAug) );
	  }
	}
  //**************************	  added by NYC	****************************************************//	  
...
//**************************  added by NYC	****************************************************//	  
    //update entries in the PARDISO aug sys (for NLP), update B and D and also diag part (so we do not need above part)
    map<int,int>::iterator it2;
    for(it2=elementMap_BD.begin(); it2!=elementMap_BD.end(); it2++)
      M[it2->second] = eltsMsys[it2->first];	  
    }
  }
//**************************	added by NYC  ****************************************************//	  
...
//  	negEigVal=-10000;
//  else
  //return number of negative eigVal
  negEigVal=MAX(numOfNegEigVal,iparm[22]);
...
void PardisoSolver::solve( OoqpVector& rhs_in )
{
  ...
  //**************************  added by NYC	****************************************************//		
  //*******************  print out rhs for matlab
#if 0
			double *Rhs_temp = rhs.elements();
  
			FILE* outfile = fopen( "test1stIterRhs_Pardiso_.txt", "wr" );

			int findkk=0;
			fprintf(outfile,"Rhs_ = [ ");
			for( int i = 0; i < n; i++ ) {
			  if(findkk%10==0){
				fprintf(outfile,"%10.16f, ... \n", Rhs_temp[i]);
			  }	else{
				fprintf(outfile,"%10.16f,", Rhs_temp[i]);
			  }
			  findkk++;
			}
			fprintf(outfile,"]; \n\n");
