...
// - 3: no refinement, do not compress
				 ...
				 int /* level */)
{
  ...
  if(idx >= 100 && phi_min_history[idx] >= .5 * phi_min_history[idx-30]) {
    stop_code = UNKNOWN;
    printf("hehe dnorm=%g rnorm=%g artol=%g\n", rnorm, dnorm, artol);
  }

  if(idx >= 100 && rnorm / dnorm > artol && 
  ...
  //onSafeSolver=1;
  //}  
  //gLackOfAccuracy=-1; //disable iter refin in sNlpLinsysRootAug
